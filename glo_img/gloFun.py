...
# 검색어 체크키워드 가져오는 함수
def getKeywordGoogleKr():
    # conn.query("set character_set_results=utf8;")
    result = None
    with conn.cursor() as curs:
        sql = "SELECT k_title, k_cnt_id, n_idx, k_nat FROM k_word where k_state = 1 and k_key = 1 and k_nat = 'kr';"
        curs.execute(sql)
        result = curs.fetchall()
...
def getKeywordGoogle():
    ...
    result = None
    with conn.cursor() as curs:
        sql = "SELECT k_title, k_cnt_id, n_idx, k_nat FROM k_word where k_state = 1 and k_key = 1 and k_nat is not null order by k_nat asc;"
        curs.execute(sql)
        result = curs.fetchall()
...
# dailyosp_url 가져오는 함수
def getDailyHostUrl():
    with conn.cursor() as curs:
        sql = "select host_url, cnt_del_date from cnt_f_list where cnt_osp = 'dailymotion' and cnt_del_act = 1 and cnt_chk!=4;"
        curs.execute(sql)
        result = curs.fetchall()
...
def getTudouUrl():
    with conn.cursor() as curs:
        sql = "select host_url, cnt_del_date from cnt_f_list where cnt_osp = 'tudou' and cnt_del_act = 1 and cnt_chk != 4 and cnt_regdate >= '2019-11-01 00:00:00' order by cnt_regdate desc;"
        # sql = "select host_url, cnt_del_date from cnt_f_list where cnt_osp in ('tudou', 'iqiyi') and cnt_regdate >= '2019-10-01 00:00:00' and cnt_regdate <= '2019-10-31 23:59:59' and cnt_del_act = 1 and cnt_chk != 4;"
        curs.execute(sql)
        result = curs.fetchall()
...
def getHostUrl():
    with conn.cursor() as curs:
        sql = "select host_url, cnt_del_date from cnt_f_list where cnt_osp not in ('google', 'dailymotion') and cnt_del_act = 1 and cnt_chk!=4;"
        # sql = "select host_url, cnt_del_date from cnt_f_list where cnt_osp in ('iqiyi', 'tudou') and cnt_del_act = 1 and cnt_chk != 4;"
        curs.execute(sql)
        result = curs.fetchall()
...
def getGoogleHostUrl():
    with conn.cursor() as curs:
        sql = "select host_url, cnt_del_date from cnt_f_list where cnt_osp = 'google' and cnt_chk=1 order by n_idx desc;"
        # sql = "select host_url, cnt_del_date from cnt_f_list where cnt_osp = 'google' and cnt_chk=1 and cnt_regdate >= '2019-10-01 00:00:00' and cnt_regdate <= '2019-10-31 23:59:59' order by cnt_regdate asc;"
        curs.execute(sql)
        result = curs.fetchall()
...
def getImage():
    ...
    result = None
    with conn.cursor() as curs:
        sql = "SELECT k_p_img, k_cnt_id, n_idx FROM k_word where k_state = 1 and k_key = 1 and k_cp_id = 'coacoa' and k_p_img is not null;"
        curs.execute(sql)
        result = curs.fetchall()
