...
def startCrawling():
    i = 0;check = True
    link = 'https://yeuphimmoi.com/quoc-gia/han-quoc/page/'
    while check:
        i = i+1
        if i == 30:
            break
        ...
        try:
            for item in article:
                imgUrl = item.find('img')['data-lazy-src']
                url = item.find('a')['href']
                titleSub = item.find('div', 'ypm-post-title').text.strip()
                if titleSub.find('(') != -1:
                ...
                c = r.content
                soup = BeautifulSoup(c,"html.parser")
                li = soup.find('ul', 'ypm-list-eps').find_all('li', 'ypm-episode')
...
if __name__=='__main__':
    ...
    print("yeuphimmoi 크롤링 시작")
    startCrawling()
    print("yeuphimmoi 크롤링 끝")
