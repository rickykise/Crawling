...
def startCrawling(url, keyItem):
    ...
    try:
        ...
        c = r.content
        soup = BeautifulSoup(c,"html.parser")
        if soup.find('li', 'list-inline-item'):
            ...
            keyU = url2.split('play/')[1].split('/')[0]
            url2 = 'https://yt.tclup.com/play.php?s='+keyU
            url2 = 'http://yt.tclup.com/play.php?s=running%20man'
            r = requests.get(url2)
            c = r.content
            soup = BeautifulSoup(c,"html.parser")
            li = soup.find('ul', id='lightSlider').find_all('li')

            for item in li:
                host_url = 'https://www.youtube.com/embed/'+item.find('a')['id']
                title = cnt_title+'_'+item.find('a')['id']
                title_null = titleNull(title)
