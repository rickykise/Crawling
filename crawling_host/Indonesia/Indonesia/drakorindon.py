...
def startCrawling(site):
    i = 0;check = True
    link = 'https://drakorindon.com/'+site+'/page/'
    while check:
        i = i+1
        if i == 30:
            break
        ...
        div = soup.find('div', 'video-content').find_all('div', 'video')

        try:
            for item in div:
                imgUrl = item.find('img')['data-src']
                url = item.find('div', 'title').find('a')['href']
...
if __name__=='__main__':
    ...
    print("drakorindon 크롤링 시작")
    site = ['ongoing','complete']
    for s in site:
        startCrawling(s)
    print("drakorindon 크롤링 끝")
