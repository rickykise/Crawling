...
def getTest():
    ...
    with conn.cursor() as curs:
        sql = "SELECT portal_title, portal_idx FROM `union`.score_data order by writeDate desc limit 50;"
        curs.execute(sql)
...
def getReply():
    ...
    with conn.cursor() as curs:
        ...
        sql = "SELECT community_title, community_idx FROM `union`.community_data where community_name = 'dcinside' and textType is null order by createDate desc;"
        curs.execute(sql)
        result = curs.fetchall()
...
def autoTextUpdate(data):
    ...
    curs = conn.cursor(pymysql.cursors.DictCursor)
    try:
        sql = 'update `union`.community_data set textType=%s, replynum=%s where community_idx=%s;'
        curs.execute(sql,(data['textType'],data['replynum'],data['community_idx']))
        conn.commit()
    finally:
        conn.close()
