...
def startCrawling():
    i = 0;check = True
    link = 'https://newtoon26.com/toon/continue/week/'
    ...
    for v in val:
        ...
        try:
            for item in div:
                ...
                span_text = item.find('a', 'bo_tit').find('span').text.strip()
                title = item.find('a', 'bo_tit').text.replace(span_text, '').strip()
                ...
                for item in div:
                    ...
                    if title_num.find('프롤로그') != -1:
                        title_num = title_num.split('\t')[0].strip()

                    data = {
                        'craw_osp_id': 'newtoon26',
                        'craw_domain': 'com',
                        'craw_title': title,
                        'craw_site_url' : url,
                        'craw_url': craw_url,
...
if __name__=='__main__':
    ...
    print("newtoon26 크롤링 시작")
    startCrawling()
    print("newtoon26 크롤링 끝")
