...
def startCrawling(param):
    i = 0;check = True
    link = "http://www.qzql.cn/index.php?m=vod-list-id-{}-pg-{}-order--by--class-0-year-0-letter--area-%E9%9F%A9%E5%9B%BD-lang-.html"
    while check:
        i = i+1
        if i == param[1]:
            break
        ...
        try:
            for item in li:
                url = 'http://www.qzql.cn'+item.find('a')['href']
                ...
                for div in divs:
                    playList = ['http://www.qzql.cn'+a['href'] for a in div.find_all('a')]
