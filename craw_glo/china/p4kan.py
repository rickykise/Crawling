...
def startCrawling():
    i = 0;check = True
    link = 'http://p4kan.com/?m=vod-type-id-14-pg-{}.html'
    while check:
        i = i+1
        if i == 30:
            break
        try:
            ...
            for item in li:
                ...
                site_url = 'http://p4kan.com'+item.find('a')['href']
                r = requests.get(site_url)
                c = r.content
                soup = BeautifulSoup(c,"html.parser")

                url = 'http://p4kan.com'+soup.find('div','videourl').find_all('a')[0]['href']
                r = requests.get(url)
                c = r.content
                soup = BeautifulSoup(c,"html.parser")
                script = soup.find('div','player').find('script')
                playList = ['http://p4kan.com'+p['href'] for p in soup.find('div','videourl clearfix').find_all('a')]
