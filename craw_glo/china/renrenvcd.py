...
def startCrawling():
    i = 0;check = True
    link = 'https://www.renrenvcd.com/rr/21-{}.html'
    while check:
        i = i+1
        if i == 12:
            break
        ...
        try:
            for item in li:
                ...
                url = 'https://www.renrenvcd.com'+item.find('a')['href']
                r = requests.get(url)
                c = r.content
                soup = BeautifulSoup(c,"html.parser")
                sub = soup.find('div','col-lg-wide-75 col-xs-1').find_all('a',target="_self")
                for item in sub:
                    host_url = 'https://www.renrenvcd.com'+item['href']
...
if __name__=='__main__':
    ...
    print("renrenvcd 크롤링 시작")
    startCrawling()
    print("renrenvcd 크롤링 끝")
