...
def startCrawling():
    i = 0;check = True
    link = 'http://rrkb.cc/index.php?m=vod-list-id-14-pg-{}-order--by--class-0-year-0-letter--area-%E9%9F%A9%E5%9B%BD-lang-.html'
    while check:
        i = i+1
        if i == 12:
            break
        ...
        try:
            for item in li:
                ...
                site_url = 'http://rrkb.cc'+item.find('a')['href']
                r = requests.get(site_url)
                c = r.content
                soup = BeautifulSoup(c,"html.parser")

                url = 'http://rrkb.cc'+soup.find('div',id = 'vlink_1').find('a')['href']
                if url.find('vod-down-id') != -1:
                    continue

                r = requests.get(url)
                c = r.content
                soup = BeautifulSoup(c,"html.parser")
                script = soup.find('div','stui-player').find('script')
                playList = ['http://rrkb.cc'+p['href'] for p in soup.find('ul','stui-content__playlist').find_all('a')]
