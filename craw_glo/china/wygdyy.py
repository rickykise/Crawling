...
def startCrawling(site):
    i = 0;check = True
    link = 'http://www.wygdyy.com/vod-list-id-{}-pg-{}-order--by--class-0-year-0-letter--area-%E9%9F%A9%E5%9B%BD-lang-.html'
    while check:
        i = i+1
        if i == site[1]:
            break
        try:
            ...
            li = soup.find('div','index-area clearfix').find_all('li')
            ...
            for item in li:
                ...
                site_url = 'http://www.wygdyy.com'+item.find('a')['href']
                r = requests.get(site_url)
                c = r.text
                soup = BeautifulSoup(c,"html.parser")

                url = 'http://www.wygdyy.com'+soup.find('div','videourl').find_all('a')[0]['href']
                r = requests.get(url)
                c = r.text
                soup = BeautifulSoup(c,"html.parser")
                script = soup.find('div','player').find('script')
                playList = ['http://www.wygdyy.com'+p['href'] for p in soup.find('div','videourl clearfix').find_all('a')]
