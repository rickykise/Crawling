...
def startCrawling():
    i = 0;check = True;a = 1
    link = 'https://www.movieffm.net/genre/krdrama/page/'
    while check:
        i = i+1
        if i == 30:
            break
        ...
        try:
            for item in article:
                ...
                for item in li:
                    host_url = 'https://betvigososyal.com/go/'+item.find('a')['href']
                    host_url = urllib.parse.unquote(host_url)
                    title = titleSub+'_'+item.find('div', 'numerando').text.replace(' ', '').strip()
...
if __name__=='__main__':
    ...
    print("movieffm 크롤링 시작")
    startCrawling()
    print("movieffm 크롤링 끝")
