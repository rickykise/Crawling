...
def startCrawling():
    i = 0;check = True
    link = 'https://xemphimhayz.net/quoc-gia/phim-han-quoc/page/'
    while check:
        i = i+1
        if i == 30:
            break
        ...
        try:
            for item in article:
                ...
                if imgUrl.find('googleusercontent') != -1:
                    imgUrl = imgUrl.split('url=')[1]
                    imgUrl = urllib.parse.unquote(imgUrl)
                url = item.find('a')['href']
                titleSub = item.find('div', 'itfs-post-title').text.strip()
                ...
                c = r.content
                soup = BeautifulSoup(c,"html.parser")
                url2 = soup.find('div', 'itfs-watch-box').find('a')['href']
