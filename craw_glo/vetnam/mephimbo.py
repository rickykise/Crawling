...
def startCrawling():
    i = 0;check = True
    link = 'https://mephimbo.com/phim-han-quoc?page={}'
    while check:
        i = i+1
        if i == 16:
            break
        ...
        try:
            for item in div:
                if item.find('a'):
                    ...
                    for j in range(1,tapNum):
                        host_url = url+'/tap-'+str(j)
                        title_num = str(j)
...
if __name__=='__main__':
    ...
    print("mephimbo 크롤링 시작")
    startCrawling()
    print("mephimbo 크롤링 끝")
