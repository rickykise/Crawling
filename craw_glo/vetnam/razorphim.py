...
def startCrawling():
    i = 0;check = True
    link = 'https://razorphim.com/phimbo/page/'
    while check:
        i = i+1
        if i == 30:
            break
        ...
        try:
            for item in article:
                ...
                c = r.content
                soup = BeautifulSoup(c,"html.parser")
                ul = soup.find_all('ul', 'episodios')

                for item in ul:
                    li = item.find_all('li')
                    for item in li:
                        host_url = item.find('div', 'episodiotitle').find('a')['href']
                        titleNum = item.find('div', 'numerando').text.strip()
...
if __name__=='__main__':
    ...
    print("razorphim 크롤링 시작")
    startCrawling()
    print("razorphim 크롤링 끝")
