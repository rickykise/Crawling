...
def startCrawling():
    i = 0;check = True
    link = 'https://maxphim.net/country/phim-han-quoc/page-'
    while check:
        i = i+1
        if i == 30:
            break
        ...
        try:
            for item in li:
                ...
                c = r.content
                soup = BeautifulSoup(c,"html.parser")
                li = soup.find('ul', id='list_episodes').find_all('li')

                for item in li:
                    host_url = item.find('a')['href']
                    titleNum = item.find('a').text.strip()
                    if titleNum.find('Preview') != -1 or titleNum.find('preview') != -1:
                        continue
