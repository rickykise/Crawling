...
def startCrawling():
    i = 0;check = True
    link = 'https://goldphim.com/phim-bo/han-quoc-phim-bo/page/'
    while check:
        i = i+1
        if i == 30:
            break
        ...
        try:
            for item in article:
                ...
                post_id = soup.find('div', 'rate-this').find('div')['data-id']

                ajax_url = 'https://goldphim.com/wp-admin/admin-ajax.php'
                ...
                for item in ul:
                    li = item.find_all('li')
                    for item in li:
                        sub_url = url2.split('tap-')[0].strip()
                        tap = item.find('span')['data-episode']
                        ser = item.find('span')['data-server']
                        host_url = sub_url+'tap-'+tap+'-server-'+ser+'/'
...
if __name__=='__main__':
    ...
    print("goldphim 크롤링 시작")
    startCrawling()
    print("goldphim 크롤링 끝")
