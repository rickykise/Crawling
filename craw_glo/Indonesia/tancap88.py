...
def startCrawling():
    i = 0;check = True
    link = 'https://tancap88.me/drama-korea/'
    while check:
        i = i+1
        if i == 30:
            break
        ...
        div = soup.find_all('div', 'tv-show')

        try:
            for item in div:
                imgUrl = item.find('img')['data-lazy-src']
                url = item.find_all('a', 'masvideos-LoopTvShow-link')[1]['href']
                titleSub = item.find_all('a', 'masvideos-LoopTvShow-link')[1].text.strip()
                if titleSub.find('(') != -1:
