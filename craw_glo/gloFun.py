...
def getCpID(cnt_id):
    ...
    with conn.cursor() as curs:
        try:
            sql = "SELECT k_cp_id FROM sbs.k_word where k_cnt_id = %s limit 1;"
            curs.execute(sql,(cnt_id))
...
def insertALL(data):
    ...
    data['cnt_cp_id'] = getCpID(data['cnt_id'])[0]
...
# 검색어 체크키워드 가져오는 함수
def groupCheck(osp_id):
    # conn.query("set character_set_results=utf8;")
    result = None
    with conn.cursor() as curs:
        sql = "select osp_id, osp_url from osp_list where osp_group = (SELECT osp_group FROM osp_list where osp_id=%s) order by osp_regdate desc limit 1;"
        curs.execute(sql,(osp_id))
        result = curs.fetchall()
        ...
        for i in range(len(result)):
            returnValue['id'] = result[i][0]
            returnValue['url'] = result[i][1]
...
def getGoogleKeyword():
    ...
    with conn.cursor() as curs:
        try:
            ...
            sql = 'SELECT k_title, k_cnt_id, k_start, k_end FROM k_word_google order by k_regdate asc;'
            curs.execute(sql)
            result = curs.fetchall()
...
# 검색어 체크키워드 가져오는 함수
def getGoogleNewKeyword():
    result = None
    conn = pymysql.connect(host='49.247.5.160',user='otogreen',password='uni1004!@',db='sbs',charset='utf8')
    curs = conn.cursor(pymysql.cursors.DictCursor)
    with conn.cursor() as curs:
        try:
            sql = "SELECT k_title, k_cnt_id FROM k_word where k_cp_id = 'sbscp' and k_state = 1 and k_key = 1 and k_nat is not null order by k_regdate desc;"
            curs.execute(sql)
            result = curs.fetchall()
...
def ex_del():
    ...
    with conn.cursor() as curs:
        try:
            sql = "SELECT k_title FROM ex_key where k_nat in ('cn', 'us')"
            curs.execute(sql)
            result = curs.fetchall()
            a = [i[0] for i in result]
        finally:
