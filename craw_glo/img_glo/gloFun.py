...
def getGoogleHostUrl():
    with conn.cursor() as curs:
        # sql = "select host_url, cnt_del_date from cnt_f_list_backup where cnt_osp = 'google' and cnt_del_act = 1 and cnt_chk!=4;"
        sql = "select host_url, cnt_del_date from cnt_f_list_backup where cnt_osp = 'google' order by n_idx desc;"
        curs.execute(sql)
...
#     with conn.cursor() as curs:
#         sql = "select host_url, cnt_del_date from cnt_f_list_backup where cnt_del_act=1 and cnt_chk!=4 and cnt_osp=%s;"
...
def getYoukuHostUrl():
    with conn.cursor() as curs:
        sql = "select host_url, cnt_del_date from cnt_f_list_backup where cnt_osp = 'youku' and cnt_del_act = 1 and cnt_chk!=4;"
        curs.execute(sql)
        result = curs.fetchall()
...
def getHostUrl():
    with conn.cursor() as curs:
        sql = "select host_url, cnt_del_date from cnt_f_list_backup where cnt_del_act = 1 and cnt_chk!=4;"
        curs.execute(sql)
        result = curs.fetchall()
...
# cnt_f_list_backup DB 업데이트 함수
def dbUpdate(checkNum,now,host_url):
    if checkNum == 1:
        sql = "update cnt_f_list_backup set cnt_chk=4, cnt_regdate2=%s, cnt_dend_act=1, cnt_dend_date=%s where host_url=%s;"
        curs.execute(sql,(now,now,host_url))
        conn.commit()
    else:
        sql = "update cnt_f_list_backup set cnt_regdate2=%s where host_url=%s;"
...
def getdailytest():
    with conn.cursor() as curs:
        sql = "SELECT host_url FROM sbs.cnt_f_list_backup where cnt_osp = 'dailymotion' order by cnt_regdate desc;"
        curs.execute(sql)
        result = curs.fetchall()
        a = [i[0] for i in result]
        # print(a)
...
def getOspTestUrl():
    with conn.cursor() as curs:
        sql = "select host_url from cnt_f_list_backup where host_url like '%ifeng.com%';"
        curs.execute(sql)
        result = curs.fetchall()
        a = [i[0] for i in result]
        # print(a)
