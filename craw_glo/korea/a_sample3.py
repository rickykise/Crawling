...
def startCrawling(site):
    i = 0;check = True;cnt_osp = 'gq8.oguogu'
    ...
    if group_id != None:
        ...
        link = 'https://gq8.oguogu.net/bbs/board.php?bo_table='+site+'&page='
    while check:
        i = i+1
        if i == 30:
            break
        ...
        li = soup.find_all('li', 'bo_liimg')

        try:
            for item in li:
                url = item.find('div', 'bo_subject').find('a')['href']
                if url.find('&page=') != -1:
                    url = url.split('&page')[0]
                title = item.find('div', 'bo_subject').find('a').text.strip()
...
if __name__=='__main__':
    ...
    print("gq8.oguogu 크롤링 시작")
    site = ['enter_new', 'enter', 'drama_new', 'drama']
    for s in site:
        startCrawling(s)
    print("gq8.oguogu 크롤링 끝")
