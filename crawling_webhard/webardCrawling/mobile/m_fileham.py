...
def startCrawling(site):
    ...
    with requests.Session() as s:
        while check:
            ...
            div = soup.find_all('div', 'data_img_box2')

            try:
                for item in div:
                    if item.find('div', 'adult'):
                        continue
                    cnt_num = item['onclick'].split("load_view(")[1].split(");")[0]
                    cnt_price = item.find('li', 'point').text.replace(",","").split("P")[0].strip()
                    ...
                    if keyCheck2['m'] == None:
                        continue
                    cnt_vol = soup.find_all('span', 'mar_left10')[3].text.strip()
                    cnt_writer = soup.find('div', 'vimgbx2').find_all('li')[2].text.split('판매자')[1].strip()
