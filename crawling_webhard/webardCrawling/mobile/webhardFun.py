...
def titleNull(title):
    ...
    title = title.replace('.', '').replace('[', '').replace(']', '').replace('(', '').replace(')', '').replace('-', '').replace('_', '').replace('NEXT', '').replace(",","").replace("'", '').replace('"', '').replace('mp4', '').replace('avi', '').replace('mkv', '').replace('�', '').replace('★', '').replace('◈', '').replace('━', '').replace('【', '').replace('】', '').replace('!', '').replace('ㅡ', '').replace('─', '').replace("+","").replace('?', '').replace(' ', '').replace('●', '')
...
def getKeywordNet():
    ...
    with conn.cursor() as curs:
        ...
        sql = "SELECT k_title, k_L_id, cnt_price FROM cnt_keywordprice where k_mcp not in ('jaye', 'warner', 'kcopa') and k_state = 1 and k_key = 1;"
        curs.execute(sql)
        result = curs.fetchall()
...
def getKeywordMong():
    ...
    with conn.cursor() as curs:
        # sql = "SELECT k_title, k_cp, cnt_price FROM cnt_keyprice where k_mcp in ((SELECT cp_mcp FROM cp_list where cp_state = 1 and cp_mcp in ('kbs', 'under'))) and k_state = 1;"
        sql = "SELECT k_title, k_L_id, cnt_price FROM cnt_keywordprice where k_mcp != 'kcopa' and k_state = 1 and k_key = 1;"
        curs.execute(sql)
        result = curs.fetchall()
...
def getKeyword():
    ...
    with conn.cursor() as curs:
        # sql = "SELECT k_title, k_cp, cnt_price FROM cnt_keyprice where k_mcp in ((SELECT cp_mcp FROM cp_list where cp_state = 1 and cp_mcp in ('kbs', 'under'))) and k_state = 1;"
        sql = "SELECT k_title, k_L_id FROM k_word where k_mcp not in ('jaye', 'warner') and k_state = 1 and k_key = 1 order by k_mcp asc;"
        curs.execute(sql)
        result = curs.fetchall()
...
# kbs, sbs 체크키워드 가져오는 함수
def getKeywordSK():
    # conn.query("set character_set_results=utf8;")
    result = None
    with conn.cursor() as curs:
        # sql = "SELECT k_title, k_cp, cnt_price FROM cnt_keyprice where k_mcp in ((SELECT cp_mcp FROM cp_list where cp_state = 1 and cp_mcp in ('kbs', 'under'))) and k_state = 1;"
        sql = "SELECT k_title, k_L_id FROM k_word where k_mcp in ('sbs', 'kbs') and k_state = 1 and k_key = 1;"
        curs.execute(sql)
        result = curs.fetchall()
...
def getKeywordSbs():
    ...
    with conn.cursor() as curs:
        # sql = "SELECT k_title, k_cp, cnt_price FROM cnt_keyprice where k_mcp in ((SELECT cp_mcp FROM cp_list where cp_state = 1 and cp_mcp in ('kbs', 'under'))) and k_state = 1;"
        sql = "SELECT k_title, k_L_id FROM k_word where k_mcp = 'sbs' and k_state = 1 and k_key = 1;"
        curs.execute(sql)
        result = curs.fetchall()
...
#     result = None
#     with conn.cursor() as curs:
#         # sql = "SELECT * from(SELECT k_title, k_L_id FROM site.k_word where k_mcp != 'jaye' and k_state = 1 and k_key = 1 UNION ALL SELECT k_title, k_cnt_id FROM sbs.k_word where k_state = 1 and k_key = 1) x"
