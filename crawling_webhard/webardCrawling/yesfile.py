...
def getContents(url):
    ...
    # print(title)
    price_check = soup.find('div', 'td_point').find('li', 'text').text.strip()
    if price_check.find('할인') == -1:
        ...
        cnt_price = soup.find('div', 'td_point').find('li', 'text').text.split("P")[1].split("P")[0].replace(" ","").replace(",","").strip()
        cnt_vol = soup.find('div', 'td_point').find('li', 'text').text.replace(" ","").split(")")[1].split("/")[1].strip()
...
if __name__=='__main__':
    ...
    site = ['BD_UC&sec=3','_BD&sec=0','BD_MV&sec=1','BD_DM&sec=2','BD_AN&sec=5']
    for s in site:
        startCrawling(s)
    print("yesfile 크롤링 끝")
