...
def getKeyword():
    ...
    with conn.cursor() as curs:
        ...
        sql = "SELECT k_title, k_L_id FROM k_word where k_mcp != 'jaye' and k_mcp != 'warner' and k_state = 1 and k_key = 1;"
        curs.execute(sql)
        result = curs.fetchall()
...
def getKeywordNet():
    ...
    with conn.cursor() as curs:
        # sql = "SELECT k_title, k_cp, cnt_price FROM cnt_keyprice where k_mcp in ((SELECT cp_mcp FROM cp_list where cp_state = 1 and cp_mcp in ('kbs', 'under'))) and k_state = 1;"
        sql = "SELECT k_title, k_L_id, cnt_price FROM cnt_keywordprice where k_mcp != 'jaye' and k_state = 1 and k_key = 1;"
        curs.execute(sql)
        result = curs.fetchall()
