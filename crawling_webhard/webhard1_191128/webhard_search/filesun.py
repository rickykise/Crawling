...
def startCrawling(key):
    ...
    with requests.Session() as s:
        ...
        link = "http://www.filesun.com/disk/board.php?board=1&listmode=all&sf=subject&sw="+encText+"&exceptadult=1&page="
        while check:
            i = i+1
            if i == 4:
                break
            ...
            try:
                for item in tr:
                    ...
                    #     dbResult = insertDB('filesun',title,title_null,url)
                    #     continue
                    cnt_writer = soup2.find('td', colspan='2').text.strip().replace(" ","").replace("\t", "")
