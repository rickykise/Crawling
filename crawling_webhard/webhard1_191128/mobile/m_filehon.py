        ...
        while check:
            ...
            try:
                for item in li:
                    ...
                    url_sub = 'http://m.filehon.com'+item.find('a')['href'].replace(" ","")
                    cnt_num = url_sub.split("&idx=")[1].split("&")[0]
                    url = 'http://m.filehon.com/?doc=board_view&idx='+cnt_num
                    ...
                    cnt_price = soup.find('dd', 'price').text.split("P")[0].replace(",","").strip()
