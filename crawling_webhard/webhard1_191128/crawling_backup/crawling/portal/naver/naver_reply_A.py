...
def getSearchUrl(conn,curs):
    with conn.cursor() as curs:
        sql = 'SELECT url FROM news_data where news_state = 1 and news_type = "A" order by createDate desc;'
        curs.execute(sql)
...
if __name__=='__main__':
    ...
    print("NEWS A 댓글 크롤링 시작")
    for u in getUrl:
        main(u)
    print("NEWS A 댓글 크롤링 끝")
