...
def fetch_feed(pageKey):
    ...
    for idx,value in enumerate(post_json['data']):
        try:
            ...
            conn = pymysql.connect(host='61.82.113.197',user='soas',password='qwer1234',db='union',charset='utf8')
            # conn2 = pymysql.connect(host='localhost',user='root',password='qwer1234',db='union',charset='utf8')
            curs = conn.cursor(pymysql.cursors.DictCursor)
            ...
            # if GetCreateDate == False:
                # insert(conn,pageKey,setContent(value['message']),value['link'],str(value['likes']['summary']['total_count']),str(value['comments']['summary']['total_count']),str(value['shares']['count']),view_get,setDate(value['created_time']))
                # insert(conn2,pageKey,setContent(value['message']),value['link'],str(value['likes']['summary']['total_count']),str(value['comments']['summary']['total_count']),str(value['shares']['count']),view_get,setDate(value['created_time']))
