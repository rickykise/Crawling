...
def getContents(url):
    ...
    cnt_price = soup.find('div', 'fsview_table').find('span', id='chkPacket').text.strip().replace(',', '')
...
def startCrawling(site):
    i = 0;check = True
    link = "http://bbs.pdpop.com/board_re.php?code=F_"+site
    print(link)
    ...
    try:
        while check:
            ...
            # if i == 4:
            #     break
            html = driver.find_element_by_class_name("sub_rarea").get_attribute('innerHTML')
            ...
            try:
                for item in li:
                    ...
                    try:
                        ...
                        conn.close()
                driver.find_element_by_xpath('//*[@id="Contents_Next"]').click()
                time.sleep(2)
            except:
                continue
