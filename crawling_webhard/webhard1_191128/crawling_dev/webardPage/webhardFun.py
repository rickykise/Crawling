...
def insertALL(data):
    ...
    connBack = pymysql.connect(host='61.82.113.197',user='autogreen',password='uni1004',db='site',port=3307,charset='utf8')
    try:
...
def getKeyword():
    ...
    with conn.cursor() as curs:
        ...
        sql = "SELECT k_title, k_L_id FROM k_word where k_mcp not in ('jaye', 'warner') and k_state = 1 and k_key = 1;"
        curs.execute(sql)
        result = curs.fetchall()
...
def getKeywordNet():
    ...
    with conn.cursor() as curs:
        # sql = "SELECT k_title, k_cp, cnt_price FROM cnt_keyprice where k_mcp in ((SELECT cp_mcp FROM cp_list where cp_state = 1 and cp_mcp in ('kbs', 'under'))) and k_state = 1;"
        sql = "SELECT k_title, k_L_id, cnt_price FROM cnt_keywordprice where k_mcp not in ('jaye', 'warner') and k_state = 1 and k_key = 1;"
        curs.execute(sql)
        result = curs.fetchall()
