...
def tpleCheck(url):
    try:
        with requests.Session() as s:
            ...
            if soup2.find('td', 'textLeft').find('img'):
                cnt_chk = 1
...
def smartfileCheck(url):
    try:
        LOGIN_INFO = {
            ...
            'id': 'Yg6Z3B1gb5sNkeQDiO0dP+1rf3GRn8+R8oGyq8b7eF0=||0QOElfh590330b2e33f8f1de1e2f67e35bc76a7',
            'id_nm': 'enjoy15',
            'login_backurl': '',
            'mode': 'login_exec',
            'pw': '7K4uHkq+tMXc1t7dsVGU5w==||Y37D5rMcb450ada6a5da4087ac49e128b7a5c6c',
            'saved_pw': 'Y',
            'ssl_mobile_flg': '1',
            'wmode': 'noheader'
        }
        ...
        login_headers = {
            ...
            'Content-Type': 'application/x-www-form-urlencoded',
            'Host': 'ssl.m.smartfile.co.kr',
            'Referer': 'http://m.smartfile.co.kr/realrank/?rnk_cate1=',
        ...
        headers = {
            ...
            'Referer': 'http://m.smartfile.co.kr/?cate1=',
            'User-Agent': 'Mozilla/5.0 (Windows Phone 10.0; Android 4.2.1; Nokia; Lumia 520) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.71 Mobile Safari/537.36 Edge/12.0',
            'X-Requested-With': 'XMLHttpRequest'
        }
        with requests.Session() as s:
            login_req = s.post('https://ssl.m.smartfile.co.kr/member/login.html', data=LOGIN_INFO, headers=login_headers)
            cnt_num = url.split('idx=')[1]
...
def me2diskCheck(url):
    try:
        ...
        with requests.Session() as s:
            ...
            cnt_priceCh = div.find_all('span', 'mar_left10')[1].text
            cnt_price = div.find_all('span', 'mar_left10')[1].text.replace(",","").split("P")[0].strip()
            if cnt_priceCh.find('이용') != -1:
                if div.find('img', 'mar_left5'):
                    jehu = div.find('img', 'mar_left5')['src']
...
def filehamCheck(url):
    try:
        ...
        cnt_price = soup.find_all('span', 'mar_left10')[1].text.strip().split('P')[0].replace(",","")
...
def bondiskCheck(url):
    try:
        ...
        c = r.content
        soup = BeautifulSoup(c,"html.parser")
        div = soup.find('div', {"class": "fileinfo_textarea"})
        cnt_chk = 0
...
def applefileCheck(url):
    ...
    try:
        ...
        if jehu == 'Y':
            cnt_chk = 1
        cnt_price = str(json_obj['cash']).replace(",","")
...
def filemaruCheck(url):
    ...
    try:
        with requests.Session() as s:
            post_one  = s.post('https://www.filemaru.com/proInclude/ajax/view.php', headers=headers, data=Page)
            soup = bs(post_one.text, 'html.parser')
            text = str(soup)
            cnt_chk = 0
...
# def main():
#     cnt_id = 'filemaru'
#     url = 'http://m.filemaru.com/proInclude/ajax/view.php16730339'
