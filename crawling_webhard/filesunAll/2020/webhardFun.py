...
def getSearchKey(conn,curs):
    with conn.cursor() as curs:
        sql = "SELECT k_title FROM k_word where k_mcp in ('kbs', 'sbs') and k_state = 1 and k_key = 1 order by k_regdate desc;"
        curs.execute(sql)
...
def getKeyword():
    result = None
    with conn.cursor() as curs:
        sql = "SELECT k_title, k_L_id FROM k_word where k_mcp in ('kbs', 'sbs') and k_state = 1 and k_key = 1;"
        curs.execute(sql)
        result = curs.fetchall()
