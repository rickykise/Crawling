...
def startCrawling(key):
    print('키워드: '+key)
    encText = urllib.parse.quote(key.encode('euc-kr','replace'))
    with requests.Session() as s:
        ...
        i = 0;check = True
        link = "http://www.tple.co.kr/storage/?s=&d=&code=&listScale=&searchTopCode=&searchTopField=title&searchTopValue="+encText+"&subSearch="
        while check:
            i = i+1
            if i == 2:
                check=False;break
            ...
            try:
                tr = soup.find_all('tr', id=re.compile("trLis+"))
