...
#         sql = "SELECT k_title FROM site.search_jaye;"
#         curs.execute(sql)
...
def getSearchKey(conn,curs):
    with conn.cursor() as curs:
        sql = "SELECT k_title FROM site.search_jaye;"
        # sql = "SELECT k_title FROM k_word where k_mcp = 'warner' and k_state = 1 and k_key = 1;"
        curs.execute(sql)
        result = curs.fetchall()
        a = [i[0] for i in result]
        # print(a)
...
def getWSearchKey():
    with conn.cursor() as curs:
        sql = "SELECT k_title FROM k_word where k_mcp = 'warner' and k_key = 1 order by n_idx asc;"
        curs.execute(sql)
        result = curs.fetchall()
        a = [i[0] for i in result]
        # print(a)
