...
# # checkNum 가져오는 함수
# def getSearchCheckNum(cnt_osp):
#     with conn.cursor() as curs:
#         sql = "SELECT DISTINCT cnt_num FROM cnt_test where cnt_osp = %s order by n_idx desc;"
...
#
# # checkNum 체크
# def checkCheckNum(cnt_num, checkNum):
#     result = None
#     for s in checkNum:
#         if cnt_num.find(s) != -1 :
#             result = s
...
def getSearchCheckNum(cnt_osp):
    with conn.cursor() as curs:
        sql = "SELECT cnt_num, cnt_osp FROM cnt_test where cnt_osp = %s order by n_idx desc;"
        curs.execute(sql, (cnt_osp))
        result = curs.fetchall()
...
def getKeyword():
    ...
    with conn.cursor() as curs:
        ...
        sql = "SELECT k_title, k_L_id FROM k_word where k_state = 1 and k_key = 1;"
        curs.execute(sql)
        result = curs.fetchall()
...
#     result = None
#     with conn.cursor() as curs:
#         sql = "SELECT k_title, k_cp, cnt_price FROM cnt_keyprice where k_mcp in ((SELECT cp_mcp FROM cp_list where cp_state = 1 and cp_mcp = 'kbs')) and k_state = 1;"
#         curs.execute(sql)
#         result = curs.fetchall()
#
#         returnValue = {}
...
#제휴 체크
def checkJehu(title,cnt_price, keyword):
    price = None
    for u in keyword.items():
        if title.find(u[0]) != -1 and cnt_price.find(str(u[1][1])) != -1 :
            price = u[1][1]
...
def insertALL(data):
    ...
    try:
        ...
        insertDB(data['Cnt_osp'],data['Cnt_title'],data['Cnt_title_null'],data['Cnt_url'])
    except Exception as e:
        print(e)
        pass
    finally :
