...
def startCrawling(key, id):
    ...
    print('키워드: '+keyword)
    i = 0;check = True
    link = 'https://torrentyoyo.com/bbs/search.php?sfl=wr_subject%7C%7Cwr_content&stx='+keyword+'&sop=and&gr_id=&srows=10&onetable=&page='
    while check:
        i = i+1
        if i == 30:
            break
        ...
        if soup.find('div', 'media-heading'):
            ...
            try:
                for item in div:
                    ...
                    if keyCheck['m'] == None:
                        continue
                    url = 'https://torrentyoyo.com/bbs/'+item.find('a')['href'].split('./')[1]
                    if url.find('&page=') != -1:
...
if __name__=='__main__':
    ...
    print("torrentyoyo 크롤링 시작")
    for k, i in getKey.items():
        startCrawling(k, i)
    print("torrentyoyo 크롤링 끝")
