...
def emoKey(key):
    ...
    key = key.replace('.', '').replace('[', '').replace(']', '').replace('(', '').replace(')', '').replace('-', '').replace('NEXT', '').replace(",","").replace("'", '').replace('"', '').replace('!', '').replace('ㅡ', '').replace("+","").replace('?', '').replace('・', '')
...
def titleNull(title):
    ...
    title = title.replace('.', '').replace('[', '').replace(']', '').replace('(', '').replace(')', '').replace('-', '').replace('NEXT', '').replace(",","").replace("'", '').replace('"', '').replace('mp4', '').replace('avi', '').replace('mkv', '').replace('!', '').replace('ㅡ', '').replace("+","").replace('?', '').replace('720p', '').replace('1080p', '').replace('・', '').replace(' ', '')
...
# 키워드 가져오는 함수
def getUnderSearchKey(conn,curs):
    with conn.cursor() as curs:
        sql = "SELECT k_title FROM site.k_word where k_mcp = 'under' order by k_mcp desc;"
        curs.execute(sql)
        result = curs.fetchall()
        a = [i[0] for i in result]
        # print(a)
...
def getKeyword(conn,curs):
    ...
    result = None
    with conn.cursor() as curs:
        sql = "SELECT k_title, k_cp FROM site.k_word where k_mcp = 'under';"
        curs.execute(sql)
        result = curs.fetchall()
