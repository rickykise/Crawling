...
def main():
    ...
    try:
        ...
        time.sleep(3)
        # 변경
        # driver.find_element_by_xpath('//*[@id="t21"]/div/button').click()
        # time.sleep(3)
        driver.find_element_by_xpath('//*[@id="MM_ASIDE_BTN"]').click()
        time.sleep(3)
        print('이제')
        driver.find_element_by_xpath('//*[@id="MM_aside_switch_version_btn"]').click()
        time.sleep(3)
        # driver.find_element_by_xpath('//*[@id="MM_aside_switch_version_btn"]').click()
        # time.sleep(3)
        driver.find_element_by_xpath('//*[@id="MM_aside_switch_version_btn"]/span').click()
        time.sleep(3)


        driver.find_element_by_xpath('//*[@id="nav"]/div[3]/nav/ul/li[2]/a').click()
        time.sleep(5)
        ...
        # arr = []
        # # if soup.find('div',{'class':'id_cui_airs_cluster','index':'1'}):
        # arr.extend(soup.find('div',{'class':'id_cui_cupid_news','index':'12'}).find_all('li','ut_item'))
        # arr.extend(soup.find('div',{'class':'id_cui_cupid_news','index':'15'}).find_all('li','ut_item'))
        # arr.extend(soup.find('div',{'class':'id_cui_cupid_news','index':'17'}).find_all('li','ut_item'))
        # arr.extend(soup.find('div',{'class':'id_cui_cupid_news','index':'20'}).find_all('li','ut_item'))


        # arr.extend(soup.find('div',{'class':'id_cui_cupid_news','index':'8'}).find_all('li','ut_item'))
        # arr.extend(soup.find('div',{'class':'id_cui_cupid_news','index':'11'}).find_all('li','ut_item'))
        # arr.extend(soup.find('div',{'class':'id_cui_cupid_news','index':'13'}).find_all('li','ut_item'))
        # arr.extend(soup.find('div',{'class':'id_cui_cupid_news','index':'16'}).find_all('li','ut_item'))

        div = soup.find_all('div', 'id_cui_cupid_news')

        for item in div:
            arr = item.find_all('li', 'ut_item')
            count = 1
            now = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            ...
            for value in arr:
                ...
                if href.find('&cluid=') != -1:
                    ...
                    driver.get(subhref)
                    time.sleep(3)
                    page_main = driver.find_element_by_class_name("ct_wrap").get_attribute('innerHTML')
                    tags = BeautifulSoup(page_main,'html.parser')
                    content = tags.find('div', 'newsct_article').text.replace("\n","").replace("\t","")
                    reporter = checkReporter(content, dbreporter)
                    data['reporter_media_name'] = reporter['m']
                    data['reporter_name'] = reporter['r']
                    ...
                    data['reporter_name'] = None
                # print(content)
                # https://n.news.naver.com/entertain/article/109/0004032449?cluid=enter_201906191020_00000099
                try:
                    if data['url'].find('article/') != -1:
                        uid = data['url'].split('article/')[1].split('?')[0]
                        uidSub = uid.split('/')[1].split('?')[0]
                        data['uid'] = uidSub
                        print(data['uid'])
                    else:
