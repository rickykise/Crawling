        ...
        while check:
            ...
            for item in article:
                ...
                try:
                    ...
                    contents = article.find('div', dir="auto", lang="und").text.replace('\n', '').strip()

                try:
                    date = article.find('div', 'css-1dbjc4n r-vpgt9t').find('div', 'r-qvutc0', dir="auto").find('span').text.strip()
                except:
                    continue
                ...
                if result:
                    ...
                    if writeDate < now.strftime('%Y-%m-%d'): break
                    if like != 0:
                        if like.find('마음에') != -1:
                            like = like.split('마음에')[0].strip()
