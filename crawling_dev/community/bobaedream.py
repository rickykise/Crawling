...
def startCrawling(key):
    ...
    site = "https://www.bobaedream.co.kr/list?code=strange&s_cate=&maker_no=&model_no=&or_gu=10&or_se=desc&s_selday=&pagescale=10&info3=&noticeShow=&s_select=Body&s_key="+key+"&level_no=&vdate=&type=list&page=1"
    context = ssl._create_unverified_context()
    # 검색된 페이지 html 가져옴
    r = requests.get(site, context=context)
    c = r.content
    soup = BeautifulSoup(c,"html.parser")
    # 키워드 페이지 총 페이지 수 확인
        ...
        for item in tr:
            ...
            if result:
                ...
                try:
                    ...
                    # insert(conn2,'bobae',data['title'],data['contents'],data['writer'],'',data['date'],dbKey[key]['add'][0],putKey,putKeyType,data['url'],data['board_number'])
                    if dbResult:
                        check=True;break
                    else:
