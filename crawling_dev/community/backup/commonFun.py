...
def getSearchKey(conn,curs):
    with conn.cursor() as curs:
        sql = 'SELECT DISTINCT keyword_main,user_idx FROM keyword_data where keyword_property=%s  and user_idx in (22,23,24,25,26,27);'
        curs.execute(sql, ('포함'))
    ...
    with conn.cursor() as curs:
        # sql = "SELECT keyword_main,keyword,user_idx FROM keyword_data where keyword_property=%s and not user_idx in (22,23,24,25,26,27) and keyword_type!=%s and keyword is not null;"
        sql = "SELECT keyword_main,keyword,user_idx FROM keyword_data where keyword_property=%s and not user_idx in (22,23,25,26,27) and keyword is not null;"
        curs.execute(sql,('포함'))
        result = curs.fetchall()
