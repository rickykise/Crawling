...
def startCrawling(key):
    print("키워드 :",key)
    i = 0;
    link = "http://www.dramameeting.com/index.php?_filter=search&mid=entertainment&search_keyword="+key+"&search_target=title_content&page="
        ...
        while check:
            ...
            for item in tr:
                ...
                try:
                    ...
                    insert(conn2,'dramameeting',data['title'],data['contents'],data['writer'],'',data['date'],dbKey[mkey]['add'][0],putKey,putKeyType,data['url'],data['board_number'])
                    if dbResult:
                        return False
                finally :
                    conn.close()
...
if __name__=='__main__':
    ...
    print("dramameeting entertainment크롤링 시작")
    for k in dbKey.keys():
        # if dbKey[k]['add'][0] == '아이유':
        #     startCrawling(k)
        startCrawling(k)
    print("dramameeting entertainment크롤링 끝")
