...
def getContents(url):
    ...
    try:
        ...
        data['board_number'] = url.split("freeb&No=")[1].split("&")[0]
...
def startCrawling(key):
    ...
    site = "http://www.bobaedream.co.kr/list.php?code=freeb&s_cate=&maker_no=&model_no=&or_gu=10&or_se=desc&s_selday=&pagescale=30&info3=&noticeShow=&bestCode=&bestDays=&bestbbs=&vdate=&ndate=&nmdate=&s_select=Subject&s_key="+key
    # 검색된 페이지 html 가져옴
    ...
    while pageNum <= pageCount:
        ...
        for item in tr:
            if item.find("td","date").text.find("/") != -1:
                check=True;break
            # if item.find("td","date").text == '04/10': check=False;break
            ...
            if result:
                ...
                try:
                    ...
                    dbResult = insert(conn,'bobae_freeb',data['title'],data['contents'],data['writer'],'',data['date'],dbKey[key]['add'][0],putKey,putKeyType,data['url'],data['board_number'])
                    insert(conn2,'bobae_freeb',data['title'],data['contents'],data['writer'],'',data['date'],dbKey[key]['add'][0],putKey,putKeyType,data['url'],data['board_number'])
                    if dbResult:
                        check=True;break
                    else:
...
if __name__=='__main__':
    ...
    print("bobaedream_freeb 크롤링 시작")
    for k in dbKey.keys():
        # if dbKey[k]['add'][0] == '벤츠':
        #     startCrawling(k)
        startCrawling(k)
    print("bobaedream_freeb 크롤링 끝")
