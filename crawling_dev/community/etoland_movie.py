...
def startCrawling(key):
    ...
    try:
        i = 0;
        link = "http://www.etoland.co.kr/bbs/board.php?bo_table=movie"
        ...
        for item in tr:
            ...
            try:
                ...
                dbResult = insert(conn,'etoland_movie',data['title'],data['contents'],data['writer'],data['writerIp'],data['date'],dbKey[mkey]['add'][0],putKey,putKeyType,data['url'],data['board_number'])
                insert(conn2,'etoland_movie',data['title'],data['contents'],data['writer'],data['writerIp'],data['date'],dbKey[mkey]['add'][0],putKey,putKeyType,data['url'],data['board_number'])
                if dbResult:
                    check=False
            finally :
                conn.close()
...
if __name__=='__main__':
    ...
    print("etoland_movie 크롤링 시작")
    for k in dbKey.keys():
        # if dbKey[k]['add'][0] == '마약왕':
        #     startCrawling(k)
        startCrawling(k)
    print("etoland_movie 크롤링 끝")
