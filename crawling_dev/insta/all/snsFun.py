...
def getSearchKey(conn,curs):
    with conn.cursor() as curs:
        sql = "SELECT keyword_main,keyword,user_idx FROM keyword_data where keyword_property=%s and keyword_state = 1 and keyword_type!=%s and keyword is not null and not keyword_main in ('쇼박스', '영화일반', '한샘') order by createDate asc;"
