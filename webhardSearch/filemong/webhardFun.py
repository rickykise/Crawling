...
def insertALLM(data):
    ...
    connBack = pymysql.connect(host='211.193.58.218',user='autogreen',password='uni1004',db='site',port=3307,charset='utf8')
    try:
        curs = connBack.cursor(pymysql.cursors.DictCursor)
        dbResult = insertM(connBack,data['Cnt_num'],data['Cnt_osp'],data['Cnt_title'],data['Cnt_title_null'],data['Cnt_url'],data['Cnt_price'],data['Cnt_writer'],data['Cnt_vol'],data['Cnt_fname'],data['Cnt_chk'])
    except Exception as e:
        print(e)
        pass
    finally :
...
def getSearchKey(conn,curs):
    with conn.cursor() as curs:
        sql = "SELECT k_title FROM k_word where k_mcp = 'jaye' and k_key = '1' and k_state = '1' order by k_regdate desc;"
        curs.execute(sql)
...
# 키워드 가져오는 함수
def getSearchKeyAsc(conn,curs):
    with conn.cursor() as curs:
        # sql = "SELECT k_title FROM k_word where k_mcp in ('sbs', 'kbs') and k_key = '1' and k_state = '1' order by k_regdate asc;"
        sql = "SELECT k_title FROM k_word where k_mcp in ('jtbc', 'jaye', 'warner') and k_key = '1' and k_state = '1' order by k_regdate asc;"
        curs.execute(sql)
        result = curs.fetchall()
        a = [i[0] for i in result]
        # print(a)
...
# 키워드 가져오는 함수
def getSearchKeyPrice():
    result = None
    conn = pymysql.connect(host='211.193.58.218',user='autogreen',password='uni1004',db='site', port=3307,charset='utf8')
    curs = conn.cursor(pymysql.cursors.DictCursor)
    with conn.cursor() as curs:
        try:
            sql = "SELECT k_title, cnt_price FROM site.cnt_keywordprice where k_mcp = 'sbs' and k_key = '1' and k_state = '1' order by n_idx asc;"
            curs.execute(sql)
            result = curs.fetchall()
...
def getKeywordW():
    ...
    with conn.cursor() as curs:
        # sql = "SELECT k_title, k_cp, cnt_price FROM cnt_keyprice where k_mcp in ((SELECT cp_mcp FROM cp_list where cp_state = 1 and cp_mcp in ('kbs', 'under'))) and k_state = 1;"
        sql = "SELECT k_title, k_L_id FROM k_word where k_mcp = 'warner' and k_state = 1 and k_key = 1;"
        curs.execute(sql)
        result = curs.fetchall()
...
def getKeywordNet():
    ...
    with conn.cursor() as curs:
        # sql = "SELECT k_title, k_cp, cnt_price FROM cnt_keyprice where k_mcp in ((SELECT cp_mcp FROM cp_list where cp_state = 1 and cp_mcp in ('kbs', 'under'))) and k_state = 1;"
        sql = "SELECT k_title, k_L_id, cnt_price FROM cnt_keywordprice where k_mcp = 'sbs' and k_state = 1 and k_key = 1;"
        curs.execute(sql)
        result = curs.fetchall()
