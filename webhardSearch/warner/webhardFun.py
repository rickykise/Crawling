...
def insertALL(data):
    ...
    try:
        ...
        dbResult = insert(connBack,data['Cnt_num'],data['Cnt_osp'],data['Cnt_title'],data['Cnt_title_null'],data['Cnt_url'],data['Cnt_price'],data['Cnt_writer'],data['Cnt_vol'],data['Cnt_fname'],data['Cnt_chk'])
...
def getSearchKeyAsc(conn,curs):
    with conn.cursor() as curs:
        sql = "SELECT k_title FROM k_word where k_mcp = 'jtbc' and k_key = '1' and k_state = '1' order by k_regdate asc;"
        curs.execute(sql)
        result = curs.fetchall()
        a = [i[0] for i in result]
        # print(a)
