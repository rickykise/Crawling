...
def startCrawling(site):
    ...
    with requests.Session() as s:
        ...
        while check:
            ...
            if i == 10:
                break
            link = 'https://www.netfile.co.kr/media/list.jsp?absolutePage='
            link2 = '&str=1&gubun='+site
            post_one  = s.get(link+str(i)+link2)
            soup = bs(post_one.text, 'html.parser')
            li = soup.find('div', id='mediaMovie').find_all('li')
            ...
            try:
                for item in li:
                    ...
                    title = item.find('p', 'subject').text.strip()
