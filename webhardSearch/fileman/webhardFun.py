...
def getSearchKeyAsc(conn,curs):
    with conn.cursor() as curs:
        ...
        sql = "SELECT k_title FROM k_word where k_mcp in ('sbs', 'kbs') and n_idx >= '8828' and k_key = '1' and k_state = '1' order by k_regdate asc;"
        # sql = "SELECT k_title FROM k_word where k_mcp in ('jaye', 'warner', 'jtbc') and n_idx >= '6865' and k_key = '1' and k_state = '1' order by k_regdate asc;"
        curs.execute(sql)
        result = curs.fetchall()
        a = [i[0] for i in result]
        # print(a)
