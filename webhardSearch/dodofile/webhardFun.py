...
def getSearchKey(conn,curs):
    with conn.cursor() as curs:
        sql = "SELECT k_title FROM k_word where k_mcp in ('kbs', 'sbs') and k_key = '1' and k_state = '1' order by k_regdate desc;"
        curs.execute(sql)
...
def getSearchKeyAsc(conn,curs):
    with conn.cursor() as curs:
        sql = "SELECT k_title FROM k_word where k_mcp in ('kbs', 'sbs') and k_key = '1' and k_state = '1' order by k_regdate asc;"
        curs.execute(sql)
        result = curs.fetchall()
        a = [i[0] for i in result]
        # print(a)
