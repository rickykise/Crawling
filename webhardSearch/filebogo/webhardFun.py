...
def insertALLM(data):
    ...
    try:
        curs = connOto.cursor(pymysql.cursors.DictCursor)
        curs2 = connBack.cursor(pymysql.cursors.DictCursor)
        dbResult = insertM(connOto,data['Cnt_num'],data['Cnt_osp'],data['Cnt_title'],data['Cnt_title_null'],data['Cnt_url'],data['Cnt_price'],data['Cnt_writer'],data['Cnt_vol'],data['Cnt_fname'],data['Cnt_chk'])
        insertM(connBack,data['Cnt_num'],data['Cnt_osp'],data['Cnt_title'],data['Cnt_title_null'],data['Cnt_url'],data['Cnt_price'],data['Cnt_writer'],data['Cnt_vol'],data['Cnt_fname'],data['Cnt_chk'])
    except Exception as e:
        print(e)
        pass
    finally :
...
def getSearchKey(conn,curs):
    with conn.cursor() as curs:
        sql = "SELECT k_title FROM k_word where k_mcp = 'warner' and k_key = '1' and k_state = '1' order by k_regdate desc;"
        curs.execute(sql)
