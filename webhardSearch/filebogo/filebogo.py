    ...
    with requests.Session() as s:
        while check:
            ...
            if i == 30:
                break
            try:
                link = 'https://www.filebogo.com/main/storage.php?chkUser=ehdl0126&liststate=&s_act2=ok&search_keyword=title&search='+key+'&p='
                post_one  = s.get(link+str(i), headers=headers)
                soup = bs(post_one.text, 'html.parser')
                search_text = soup.find('div', 'sub_search_txt').text.strip()
                if search_text.find('자료가 없습니다') != -1:
                    break
